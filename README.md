...
2. 보안 강화: 인증, 인가 로직을 각 마이크로서비스가 아닌 keycloak 서비스에서 중앙 집중식으로 관리하므로 보안에는 이점이 있다.
3. 사용자 관리의 편의성: 사용자, 권한, 그룹 등 사용자 인증에 필요한 모든 정보를 keycloak에서 ui 통해 관리할 수 있음
4. 다양한 인증 방식 지원: keycloak은 SSO, 2FA, 소셜 로그인 등 다양한 인증 방식을 지원함.

## 단점
1. 런타임 의존: 각 서비스를 실행하기 위해선 Keycloak 인스턴스가 반드시 올라와 있어야 함 
2. 중앙 집중식 포인트의 실패: Keycloak 서버에 문제가 발생했을 때 공개키 유효 시간에 따라 전체 서비스에 인증, 인가 문제 발생할 가능성 있음
2. 확장성 문제: Keycloak 인스턴스가 단일 장애 지점인 만큼 Keycloak 인스턴스를 잘 관리하고 필요에 따라 캐싱과 클러스터링 필요
